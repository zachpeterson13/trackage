...
% tests for the store_package:handle_call callback for store/3.
...
handle_call_store_happy() ->
  ...
  Actual1 = store_package:handle_call({store, "key1", [{"val1", "val2"}]}, some_from_pid, some_riak_pid),
...
% tests for the store_package:store/3 api function.
store_test_() ->
  [
   store_invalid_key(),
   store_invalid_value()
...
store_invalid_key() ->
  ...
  Actual1 = catch store_package:store(pid, not_a_string, [{"val", "val"}]),
  Actual2 = catch store_package:store(pid, "", [{"val", "val"}]),
...
store_invalid_value() ->
  ...
  Actual1 = catch store_package:store(pid, "key", []),
  Actual2 = catch store_package:store(pid, "key", not_a_list),
  Actual3 = catch store_package:store(pid, "key", [1, 2, 3, 4]),
  Actual4 = catch store_package:store(pid, "key", [{1, 2, 3}]),
